...
//        flowLayout.sectionInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
    ...
    lazy var forwordBgView: WBHomeForwordCellBg = {
       let view = Bundle.main.loadNibNamed("WBHomeForwordCellBg", owner: nil, options: nil)?.last as! WBHomeForwordCellBg
    ...
    lazy var picCollectionView : WBHomePicCollection = {
        let collectionView : WBHomePicCollection = WBHomePicCollection(frame: CGRect.zero, collectionViewLayout: self.collectionViewFlowLayout)
        // 布局
        ...
        didSet{
            ...
            if let url = status?.user?.profile_image_url{
                let urlStr = URL(string: url)
                 iconImageView.sd_setImage(with: urlStr)
            }
